...
class StockReservationModel
{
    ...
    {
        $db = Flight::db();
        $st = $db->prepare("SELECT COALESCE(SUM(quantite),0) AS reserved FROM stock_reservation WHERE id_article = :a AND id_depot = :d");
        $st->execute([':a' => $article, ':d' => $depot]);
        $row = $st->fetch(PDO::FETCH_ASSOC);
        return isset($row['reserved']) ? (float)$row['reserved'] : 0.0;
    }

    public static function reserve(int $article, int $depot, float $quantite, ?string $reference, int $userId): void
    {
        ...
        $st = $db->prepare("INSERT INTO stock_reservation(id_article, id_depot, quantite, reference, created_by) VALUES (:a, :d, :q, :r, :u)");
        $st->execute([':a' => $article, ':d' => $depot, ':q' => $quantite, ':r' => $reference, ':u' => $userId]);
    }

    public static function cancel(int $article, int $depot, float $quantite, ?string $reference, int $userId): void
    {
    ...
    public static function consumeForReference(int $article, int $depot, float $quantite, ?string $reference, int $userId): void
    {
        ...
        if ($reference) {
            $st = $db->prepare("SELECT COALESCE(SUM(quantite),0) AS reserved_ref FROM stock_reservation WHERE id_article=:a AND id_depot=:d AND reference=:r");
        ...
        if ($consume <= 0) { return; }
        $ins = $db->prepare("INSERT INTO stock_reservation(id_article, id_depot, quantite, reference, created_by) VALUES (:a, :d, :q, :r, :u)");
